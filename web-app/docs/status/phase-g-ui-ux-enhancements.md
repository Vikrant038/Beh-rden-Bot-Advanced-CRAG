...
| Admin pipeline tester — CRAG diagnosis | Pipeline switcher (Agentic ⇄ Standard CRAG) with per-stage diagnosis for both paths; `runStandardCrag(collectTrace)` emits a 5-stage glass-box trace (pre/cache lookup → sub-queries & hybrid retrieval → CRAG gate → grounded generation → cache write & persist) with retrieval telemetry, LLM costs, and pre/post-processing timings — zero overhead for the chat path (trace only when `collectTrace` is set). Developer mode + bypass-cache switches kept identical | `src/app/admin/pipeline-tester/page.tsx`, `src/components/admin/pipeline/pipeline-visualizer.tsx`, `src/server/rag/pipeline.ts`, `src/server/routers/admin.ts`, `tests/integration/rag-pipeline.test.ts` | ✅ |
