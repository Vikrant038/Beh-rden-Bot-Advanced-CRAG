...
export function toVectorLiteral(vector: number[]): string {
  ...
  if (vector.some((v) => typeof v !== "number" || !Number.isFinite(v))) {
    throw new Error("Invalid vector element: not a finite number");
  }
  return `[${vector.join(",")}]`;
}
...
const FTS_STOPWORDS = new Set(
  // English
  (
    "a an the and or but if then else of to in on for with by at from as is are was were be " +
    "been being have has had do does did will would can could should may might must shall not " +
    "no so than that this these those it its i you he she we they them their his her my your " +
    "our who whom which what when where how why also only just very too much more most such " +
    "each any all both some per via about into over under between during before after against " +
    // German
    "der die das den dem des ein eine einen einem einer eines und oder aber wenn dann sonst zu " +
    "in auf für mit von bei aus an als ist sind war waren sein gewesen haben hat hatte gehabt " +
    "wird werden wurde würde kann können konnte soll sollen muss müssen nicht kein keine keinen " +
    "keinem so dass diese dieser dieses es sie er wir ihr ihre ihrer ihres mein meine dein deine " +
    "seine unser unsere euer eure wer was wann wo wie warum auch nur sehr viel mehr meisten " +
    "solche jeder jede jedes jeden alle beide manche pro über unter zwischen während vor nach " +
    "gegen bis um ohne durch wegen"
  ).split(" "),
);
