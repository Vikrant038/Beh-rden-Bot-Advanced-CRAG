...
**Web app** (`web-app/.env`): `DATABASE_URL`, `MIGRATION_DATABASE_URL`, `NEXTAUTH_SECRET`/`AUTH_SECRET`, `GROQ_API_KEY`, `GROQ_MODEL`, `HF_TOKEN`, `HF_INFERENCE_URL`, `EMBEDDING_MODEL`, `RERANKER_URL`/`RERANKER_TOKEN`/`RERANKER_MODEL`, `GITHUB_CLIENT_ID/SECRET`, `GOOGLE_CLIENT_ID/SECRET`, `RESEND_API_KEY`, `LANGFUSE_*`, `UPSTASH_REDIS_*`.
